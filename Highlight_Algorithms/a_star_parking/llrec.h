...
Node* llfilter(Node* head, Comp pred)
{
    ...
    if(head == NULL){
        return head; //  at the end of the list start wrapping back up
    }
    else if(pred(head->val)){
        Node* temp = llfilter(head->next, pred);
        delete head; //  deletes the node matching pred
        return temp;
    }
    else{
        head->next = llfilter(head->next, pred); // connects to undeleted node or NULL indicating the end.
        return head;
    }
}
