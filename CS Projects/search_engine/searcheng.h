...
typedef std::map<std::string, WebPage*> database;
typedef std::map<std::string, PageParser*> parserbase;
...
private:
    ...
    //       their page/filename
    database webpages_;
    std::set<std::string> isComplete;
    std::map<std::string, WebPageSet> to_combine;
