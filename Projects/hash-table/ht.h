...
private:
   ...
   long unsigned int length_c = sizeof(CAPACITIES)/sizeof(CAPACITIES[0]);
   double resizeAlpha = 0.0;
   double num_inserted = 0.0;
};
...
{   
    if((num_inserted/CAPACITIES[mIndex_]) >= resizeAlpha){
        this->resize();
    }
    HASH_INDEX_T h = this->probe(p.first);
    if(h != npos){
        if(this->find(p.first) == nullptr){
            HashItem* to_add = new HashItem(p);
            ++num_inserted;
            table_[h] = to_add;
        }
        else{
...
{   
    ...
    if(h != npos) table_[h]->deleted = true;
}
...
{   
    ...
    else throw std::logic_error("No more CAPACITIES");
